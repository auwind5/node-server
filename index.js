  ...
  connection.beginTransaction((error) => {
    ...
    connection.query(
      ...
      (error, results, fields) => {
        ...
        connection.query(
          'update Book set bookName = ?, author = ?, shelfID = ?, city = ? where bookID = ?',
          [
...
app.post('/api/addBook', (req, res) => {
  ...
  connection.beginTransaction((error) => {
    ...
    connection.query(
      'insert into Book select max(bookId)+1 , ? , ?, ?, ? from Book',
      [req.body.bookName, req.body.author, req.body.shelfID, req.body.city],
      (error, results, fields) => {
        if (error) {
          return connection.rollback(() => {
            throw error
